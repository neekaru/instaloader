...
class Highlight(Story):
    ...
    def _fetch_items(self):
        ...
        try:
            ...
            iphone_items = data.get('reels', {}).get(reel_key, {}).get('items', [])
            self._items = []
            for it in iphone_items:
                node = {
                    'id': str(it['pk']),
                    '__typename': 'GraphStoryVideo' if it.get('media_type') == 2 else 'GraphStoryImage',
                    'display_url': it.get('image_versions2', {}).get('candidates', [{}])[0].get('url', ''),
                ...
                if it.get('media_type') == 2 and it.get('video_versions'):
                    node['video_resources'] = [
                        {'src': v['url'], 'config_width': v.get('width', 0), 'config_height': v.get('height', 0)}
    ...
    def _fetch_iphone_struct(self) -> None:
        if not (self._context.iphone_support and self._context.is_logged_in and not self._iphone_struct_):
            return
        data = self._context.get_iphone_json(
            path='api/v1/feed/reels_media/?reel_ids=highlight:{}'.format(self.unique_id),
            params={},
